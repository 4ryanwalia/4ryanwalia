...
- **Section headers, the field kit and every link chip** are generated by
  `scripts/theme_cards.py` — one prompt line per section, typing its own
  command. Edit the `SECTIONS`, `KIT` and `LINKS` tables at the top, then run
  `python scripts/theme_cards.py`. It is deterministic, so a no-op run leaves
  the repo clean. Adding a link means adding a row to `LINKS`, regenerating,
  and wrapping the new `assets/link-*.svg` in an `<a>` in the README — an SVG
  loaded through `<img>` ignores any `<a>` inside itself, which is why each
  chip is a separate file rather than one wide row.
